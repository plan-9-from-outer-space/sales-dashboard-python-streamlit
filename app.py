...
with view1:
    ...
    expander.write(data)
with dwn1:
    st.download_button(label = "Get Data", 
                       data = data.to_csv().encode("utf-8"),
                       file_name = "RetailerSales.csv", 
                       mime = "text/csv")

df["Month_Year"] = df["InvoiceDate"].dt.strftime("%b'%y")
# reset_index() creates a dataframe from a series
...
with view2:
    expander = st.expander("Monthly Total Sales")
    data = result
    expander.write(data)
with dwn2:
    st.download_button(label = "Get Data", 
...
treemap = df[["Region", "City", "TotalSales"]] \
    .groupby(by = ["Region","City"])["TotalSales"].sum().reset_index()
# print(treemap.head())
...
with view5:
    ...
    expander.write(df)
with dwn5:
    st.download_button(label = "Get Raw Data", 
